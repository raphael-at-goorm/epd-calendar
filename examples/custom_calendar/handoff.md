...
- **전체 화면**: 모드 전환 시 4사이클 clear (`g_softFlushLeft = 0`)
- **소프트 플러시**: 매 1분마다 clear 없이 재그리기 (14회 후 다시 full clear)
- **부분 플래시**: `flashArea()` — 2사이클 clear → 해당 영역만 재그리기
  - 주간 뷰: 현재시간 바가 이동할 때 (`flashTimeMarkerBand`)
  - 일간 뷰: N분후/경과 카운트다운이 바뀔 때 (`flashArea(0, 90, 960, 65)`)

---

## 가상 좌표 시스템 (주간·일간 공통)

09:00 = vY 0, 23:30 = vY 696 (= 14.5시간 × 48px/hr)

```
vY = (hour - 9) * 48 + min/60 * 48
scrollOffset = clamp(curVY - evAreaH/3, 0, VEND - evAreaH)
...
| 구분선 | nextY+4 |
| 시간 행 | nextY+22 (≈114~130) |
| 장소 행 | +43px |
| 참석자 헤더 | +43px |
| 참석자 목록 | 각 39px 간격 |

카운트다운 플래시 영역: `flashArea(0, 90, EPD_WIDTH, 65)` → y=90..155 커버

...
- **HTTP OTA**: GitHub Release 다운로드 URL의 302 redirect를 따라가도록 설정, 수면 화면에서도 OTA 체크/ArduinoOTA 처리 유지
- **잔상 복구**: 부팅 시 강한 전체 화면 복구 루틴 실행, 매일 00:00 KST대에 하루 1회 자동 복구 후 수면 화면 표시

> 주의: 2026-07-02 이전 HTTP OTA 펌웨어는 GitHub redirect를 따라가지 못해 자동 pull OTA가 실패할 수 있다.
> 해당 펌웨어가 올라간 기기는 한 번만 USB 또는 로컬 ArduinoOTA로 새 펌웨어를 올려야 이후 HTTP pull OTA가 정상 동작한다.
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-07-02 | 부팅 강제 screen repair 및 매일 자정 자동 screen repair 추가 |
| 2026-07-02 | OTA redirect follow 추가, 수면 시간 OTA 처리 유지, GitHub 예제 CI 실패 메일 방지 |
